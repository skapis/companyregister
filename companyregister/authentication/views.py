...
import threading
from userprofile.models import APILimits, Register
        ...
        if not User.objects.filter(username=username).exists():
            if not User.objects.filter(email=email).exists():
                ...
                for register in registers:
                    APILimits.objects.create(owner=user, registerName=register, limit=register.limit)
...
class VerificationView(View):
    def get(self, request, uidb64, token):
        ...
        messages.success(request, 'Account is activated, please login')
        return redirect('login')
        ...
        if username and password:
            ...
            if user:
                if user.is_active:
                    ...
                    api_limits = APILimits.objects.filter(owner=user)
